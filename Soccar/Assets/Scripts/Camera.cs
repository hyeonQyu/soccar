...
{
    public static GameObject[] _gameObjectList;

    public static GameObject _gameObject;

    private int _playerIndex; // p1=0, p2=1, p3=2, p4=3
    private const float DISTANCE = 17;
    private const int PLAYERCOUNTS = 4;
    ...
    void ChangeCameraView()
    {
        ...
        switch (PlayerController._myPlayerIndex)
        {
            case 0:
                transform.transform.position = new Vector3(_gameObject.transform.position.x, DISTANCE, _gameObject.transform.position.z - DISTANCE);
                transform.eulerAngles = new Vector3(45f, _playerIndex * 90f, 0);
                break;
            case 1:
                transform.transform.position = new Vector3(_gameObject.transform.position.x -DISTANCE, DISTANCE, _gameObject.transform.position.z);
                transform.eulerAngles = new Vector3(45f, _playerIndex * 90f, 0);
                break;
            case 2:
                transform.transform.position = new Vector3(_gameObject.transform.position.x, DISTANCE, _gameObject.transform.position.z + DISTANCE);
